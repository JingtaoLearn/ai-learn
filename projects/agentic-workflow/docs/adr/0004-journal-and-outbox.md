...
Journal every external effect before attempting it, observe the target afterward, and treat unresolved ambiguity as a legal stop rather than a retry instruction. Create logical notification events atomically with their evidence and retry only delivery transport. The authoritative approval, operation, recovery, and outbox protocol lives in [`../solution-design.md`](../solution-design.md#8-external-effect-transaction).
...
- Exactly-once physical delivery is not promised; exactly-once logical event identity is.

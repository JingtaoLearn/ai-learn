...
Read the canonical vocabulary in [`CONTEXT.md`](CONTEXT.md). The first verified product-level tracer is recorded in [`VALIDATION.md`](VALIDATION.md). The four progressively disclosed setup templates are:
...
- [`templates/EXECUTION_POOL.md`](templates/EXECUTION_POOL.md) — bounded Specialist pools, isolated workspaces, remote heavy execution, and manifest-verified synchronization.

## Hermes resource model

A dedicated functional Agent is a Hermes **Profile** and therefore can appear as a Bot. Each Profile has its own identity, model configuration, memory, Sessions, Skills, Cron jobs, and state directory. Two Agent processes never share one Profile. A concrete Specialist role may expose up to three isolated Profile instances; the Product Owner remains singleton. No static cross-role global concurrency cap is imposed.
...
`AgenticWorkflow-Assistant` is the global flow maintainer. It creates and maintains isolated Product Agent Suites but does not take over product decisions. Every product starts with one Product Owner Agent and adds a Specialist role only when an observed Gap requires an independent function. Once a role exists, independent Actions may use slots `01..03` under the shared [`EXECUTION_POOL.md`](templates/EXECUTION_POOL.md) contract.
...
4. The Owner reconciles every non-Done Workstream, builds a Ready set, and chooses the largest safe Action set across role pools. Formal work with acceptance, artifacts, blocking, retry, or review becomes a Kanban task; a short consultation may use native `message_agent`.
...
8. The Owner immediately re-enters `Goal + Principles -> Portfolio Evidence -> Workstream Gaps -> Action set`, fills newly available independent slots, and lets blocked lanes coexist with progress elsewhere. Each Action declares its Profile instance, workspace, read/write sets, semantic seams, execution host, resource class, lease and fencing identity. It records a Portfolio-level wait only when no lane can legally advance.

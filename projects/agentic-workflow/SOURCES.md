...
The Product Owner Agent chooses which sources matter for the current decision. Verified routes into its existing canonical Owner Session include user messages, exact-Session `session-messenger` envelopes, and zero-reasoning Cron output delivered to `bot-chat`. It may inspect other sources with existing Hermes tools; no ingestion service is required.
...
- Short Bot-to-Bot messages from Product Agents; use `session-messenger` when a headless sender needs an explicit exact-Session callback.
...
- Hermes Kanban tasks for formal work with acceptance, artifacts, blocking, retry, review, or crash recovery.
...
- [Kanban](https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban) — durable formal work, task correlation, blocking, handoff, and `notify+wake`.
...
- `/home/jingtao/research/hermes-agent-communication-2026-09/report.md` — 2026-09-03 local communication-mechanism research and recommended transport layering.

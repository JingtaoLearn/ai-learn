...
class _CompiledPlan:
    ...
    def candidate_capacity(self) -> int:
        return prod(len(dimension.values) for dimension in self.dimensions)
...
def _compile_plan(
    ...
    for path, definition_value in sorted(space.items()):
        ...
        values = tuple(
            _normalize_scalar(
                schemas[slot]["properties"][parameter],
                value,
...
def _suggestion(
    ...
    in_range = all(
        ...
        in {canonical_json_bytes(value) for value in dimension.values}

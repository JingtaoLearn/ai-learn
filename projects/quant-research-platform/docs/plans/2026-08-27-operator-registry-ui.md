...
1. Assert Uvicorn binds only `127.0.0.1:8090`, runs as Feng's non-root user, uses
   ...
   worker.
2. Assert the user unit `WorkingDirectory` and environment example bind
   `/home/feng/quant-platform/releases/REPLACE_WITH_RELEASE_ID`, while `ExecStart` binds
   `/home/feng/quant-platform/runtime/venv-ui-REPLACE_WITH_RELEASE_ID/bin/python`. Production mode
   is SSO and real env files are ignored.
   `/home/feng/quant-platform/current` is never accepted as the project root. Deployment must
   substitute the exact immutable release ID in the source and runtime settings; symlink-component
...
- Add reviewed user-service and placeholder environment templates that bind the exact immutable
  source release and its matching per-release runtime rather than a moving symlink.

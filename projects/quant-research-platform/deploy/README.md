...
The release must already exist under `/home/feng/quant-platform/releases/` with an executable
`.venv/bin/python`. The active user unit, private environment file, and
`state/platform/catalog.sqlite3` must exist as regular, non-symlink files.
...
release ID in the reviewed unit template. It then verifies:

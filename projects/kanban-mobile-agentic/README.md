...
The builder validates the input schema, board/status totals, unique task IDs, and the allowed task field contract. It emits only the fields needed by the viewer and redacts path-, URL-, and branch-shaped text from summaries.
...
The first validation rebuilds into a temporary file and checks HTML structure, the embedded data contract and totals, forbidden fields and path patterns, read-only/network-free JavaScript, accessible controls, required status vocabulary, and JavaScript syntax when Node.js is available. The optional real-browser validation uses installed Playwright and Chromium to exercise search, board/status filters, reset, task detail, keyboard close, mobile overflow, and the capped desktop grid; it writes visual evidence under `validation/`.
